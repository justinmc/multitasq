...
Multitasq.Modal=Backbone.View.extend({template:_.template($("script.template-modal").html()),parentSelector:".modal-view",task:null,events:{"click .modal-background":"close","click .close":"close","keyup body":"keyup"},initialize:function(t){this.app=t.app,this.task=t.task;var e=this;this.task.bind("change",function(){e.render()}),this.keyupBound=this.keyup.bind(this),$("body").on("keyup",this.keyupBound),this.render()},render:function(){return this.$el=$(this.template({title:this.task.get("title"),description:this.task.get("title"),created:this.task.getCreatedHuman(),updated:this.task.getUpdatedHuman()})),$(this.parentSelector).html(this.$el),this.editTitle=new Multitasq.EditableInput({task:this.task,attribute:"title",parentSelector:".editable-input.title"}),this.editDescription=new Multitasq.EditableInput({task:this.task,attribute:"description",parentSelector:".editable-input.description","long":!0}),this.delegateEvents(),this},show:function(){},close:function(){$("body").off("keyup",this.keyupBound),this.app.router.navigate(""),this.remove()},keyup:function(t){(27===t.which||13===t.which)&&this.close()}});
