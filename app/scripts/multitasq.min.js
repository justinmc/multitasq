...
Multitasq.Modal=Backbone.View.extend({template:_.template($("script.template-modal").html()),parentSelector:".modal-view",task:null,events:{"click .modal-background":"close","click .close":"close","click .minimize":"minimize","click .maximize":"maximize","click .up":"up","click .down":"down","keyup body":"keyup"},initialize:function(t){this.app=t.app,this.task=t.task;var i=this;this.listenTo(this.task,"change",function(){i.render()}),this.keyupBound=this.keyup.bind(this),$("body").on("keyup",this.keyupBound),this.render()},render:function(){return this.$el=$(this.template({title:this.task.get("title"),description:this.task.get("title"),upped:this.task.get("upped"),minimized:this.task.get("minimized"),created:this.task.getCreatedHuman(),updated:this.task.getUpdatedHuman()})),$(this.parentSelector).html(this.$el),this.editTitle=new Multitasq.EditableInput({task:this.task,attribute:"title",parentSelector:".editable-input.title"}),this.editDescription=new Multitasq.EditableInput({task:this.task,attribute:"description",parentSelector:".editable-input.description","long":!0}),this.task.get("title")===this.task.defaultTitle&&(this.editTitle.editing=!0,this.editTitle.render()),this.delegateEvents(),this},show:function(){},close:function(){$("body").off("keyup",this.keyupBound),this.app.router.navigate(""),this.remove()},minimize:function(){this.task.save({minimized:!0}),this.close()},maximize:function(){this.task.save({minimized:!1}),this.close()},up:function(){this.task.save({upped:!0}),this.close()},down:function(){this.task.save({upped:!1}),this.close()},keyup:function(t){(27===t.which||13===t.which)&&this.close()}});
